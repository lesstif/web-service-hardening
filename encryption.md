...
대칭키 알고리즘을 사용하는 유명한 제품은 2차 대전때 독일군이 사용한 암호화 장비인 [에니그마 머신](https://upload.wikimedia.org/wikipedia/commons/3/3e/EnigmaMachineLabeled.jpg)이 있습니다.

[U-571](http://movie.daum.net/moviedb/main?movieId=984)이라는 영화는 연합군이 독일군의 암호화 통신을 해독하기 위해 에니그마 장비를 탈취하는 특수 작전에 대해서 다루고 있습니다. 

이처럼 대칭키 방식의 최대 단점은 암호화 통신을 하기가 어렵다는 점입니다. 

가령 대칭키를 보내는 중간에 유출될 우려가 있고 이로 인해 암호화 통신을 도감청 당할 수 있으며 여러 상대방과 통신할 경우 각각의 대칭키를 관리하기(생성, 전달, 유출시 폐기)가 어렵다는 치명적인 단점이 있습니다.
...
암호화를 수행할 때 개별 블록으로 나눠진 데이타를 어떻게 처리하는 것을 블록 암호 운영 모드라고 하며 여러 가지 모드가 있습니다.
